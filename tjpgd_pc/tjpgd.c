...
)
{
	...
	call_count++;
	if (call_count > 1 && call_count < 500) {
		printf("C huffext #%u: CLEAR wbit=%u, old_wreg=%08lX, mask=%08lX, new_w=%08lX\n", 
			   call_count, wbit, (unsigned long)jd->wreg, (unsigned long)((wbit > 0) ? ((1UL << wbit) - 1) : 0), (unsigned long)w);
	}
	...
	while (wbit < 16) {	/* Prepare 16 bits into the working register */
		if (jd->marker) {
			...
			if (call_count > 1 && call_count < 500) {
				printf("C huffext #%u: marker_found=%02X, generating stuff byte 0xFF\n", call_count, jd->marker);
			}
		} else {
			...
			d = *dp++; dc--;
			if (call_count > 1 && call_count < 500) {
				printf("C huffext #%u: READ byte=%02X, dc=%zu\n", call_count, d, dc);
			}
			if (flg) {		/* In flag sequence? */
				flg = 0;	/* Exit flag sequence */
				if (d != 0) {
					...
					if (call_count > 1 && call_count < 500) {
						printf("C huffext #%u: SET MARKER 0xFF%02X\n", call_count, d);
					}
				} else {
					if (call_count > 1 && call_count < 500) {
						printf("C huffext #%u: 0xFF 00 -> escaped 0xFF\n", call_count);
					}
				}
				d = 0xFF;
			} else {
				if (d == 0xFF) {		/* Is start of flag sequence? */
					flg = 1; 
					if (call_count > 1 && call_count < 500) {
						printf("C huffext #%u: Found 0xFF, entering flag sequence\n", call_count);
					}
		...
		wbit += 8;
		if (call_count > 1 && call_count < 500) {
			printf("C huffext #%u: After adding byte: w=%08lX, wbit=%u\n", call_count, (unsigned long)w, wbit);
		}
	}
...
)
{
	...
	if (jd->marker) {	/* Generate a maker if it has been detected */
		printf("C restart: Using marker %02X from bitstream\n", jd->marker);
